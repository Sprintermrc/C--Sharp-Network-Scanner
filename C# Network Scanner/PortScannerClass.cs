...
namespace network_util
{
    public static class PortScannerClass
    {
        public static void PortScanCustom(string ip, int minPort, int maxPort)
        {
            ...
            try 
            {
                var task = RunCustomPortScanAsync(ip, minPort, maxPort);
                task.Wait();
            }
            catch (Exception ex)
            {
        ...
        public static async Task RunCustomPortScanAsync(string ip, int minPort, int maxPort){
            Console.WriteLine($"> Checking ports {minPort}-{maxPort} on {ip}....\n");
            PortScanner cps = new PortScanner(ip, minPort, maxPort);
            var progress = new Progress<PortScanner.PortScanResult>();
            progress.ProgressChanged += (sender, args) =>
            {
                if (args.IsPortOpen == true)
                {
                    Console.WriteLine(
